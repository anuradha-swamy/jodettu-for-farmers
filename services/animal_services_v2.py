...
from services import taxes,discounts,convenience_fees,vaccination_service
import os
...
class OwnAnimalServices:
    ...
    @staticmethod
    def get_breeds_by_animal_type(animal_type: str):
        """
    ...
    @staticmethod
    def bulk_import_animals(csv_content, image_files):
        try:
            csv_text = io.StringIO(csv_content.decode("utf-8"), newline="")
            ...
            for animal in animal_data:
                ...
                # await OwnAnimalServices.add_new_animal(animal_model,matched_files, user_id) 
                inserted+=1
    ...
    async def vacc_dues():
        ...
        vacc_dues = []
        for doc in docs:
            updated_doc = vaccination_service.VaccinationService.process_vaccinations(doc)
            updated_doc["_id"] = str(doc["_id"])
            
            # Handle new vaccination structure
            upcoming_vaccinations = updated_doc.get("upcoming_vaccinations", [])
            if upcoming_vaccinations:
                for vacc in upcoming_vaccinations:
                    vacc_dues.append({
                        "animal_name": updated_doc['own_animal_name'],
                        "vaccination_name": vacc.get("vaccination_name", ""),
                        "next_vaccination_date": vacc.get("next_vaccination_date", ""),
                        "breed": updated_doc["own_animal_breed"],
                        "age": updated_doc["own_animal_age"]
                    })
            else:
                # Fallback to old structure
                vacc_dues.append({
                    "animal_name": updated_doc['own_animal_name'],
                    "due_date": updated_doc.get('vaccination_due_date', ''),
                    "breed": updated_doc["own_animal_breed"],
                    "age": updated_doc["own_animal_age"]
                })
        return vacc_dues
